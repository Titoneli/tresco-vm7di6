...
class _FinanceiroTabWidgetState extends State<FinanceiroTabWidget> {
  ...
  Widget _buildAddButton() {
    return FFButtonWidget(
      onPressed: () => _showAdicionarLancamento(),
    ...
    return ListView.separated(
      ...
      itemBuilder: (ctx, i) {
        ...
        return Padding(
          ...
          child: Row(children: [
            Icon(isDespesa ? Icons.arrow_downward : Icons.arrow_upward, size: 22,
                color: isDespesa ? const Color(0xFFF56565) : const Color(0xFF48BB78)),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(d.categoria ?? d.descricao ?? '', style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w600)),
              if (d.descricao != null && d.descricao != d.categoria)
                Text(d.descricao!, style: GoogleFonts.inter(fontSize: 13, color: FlutterFlowTheme.of(context).secondaryText)),
            ])),
            Text('${isDespesa ? "- " : "+ "}${_model.formatCurrency(d.valor ?? 0)}',
  ...
  void _showAdicionarLancamento() {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: _bg,
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setSheetState) {
        final categorias = isDespesa ? despesaCategorias : entradaCategorias;
        return DraggableScrollableSheet(
          ...
          builder: (ctx, scrollCtrl) => SingleChildScrollView(
            ...
            padding: EdgeInsets.only(left: 20, right: 20, top: 20, bottom: MediaQuery.of(ctx).viewInsets.bottom + 20),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Center(child: Text('Adicionar Lançamento', style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w700))),
              const SizedBox(height: 16),
              Container(
              ...
              const SizedBox(height: 16),
              Text('Categoria', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w700)),
              const SizedBox(height: 8),
              GridView.count(
              ...
              const SizedBox(height: 16),
              Text('Valor em R\$', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w700)),
              const SizedBox(height: 8),
              TextField(
              ...
              if (isRecorrente) ...[
                ...
                _pickerField('Data do Pagamento', _model.formatDate(dataPagamento), () async {
