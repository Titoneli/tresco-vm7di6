...
class _PassageiroFormMWidgetState extends State<PassageiroFormMWidget> {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      if (_model.errorMessage != null && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(_model.errorMessage!), backgroundColor: FlutterFlowTheme.of(context).error));
      }
    });
  }
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: _model.isLoading
              ? Center(child: SpinKitPulse(color: _primary, size: 50.0))
              : Column(children: [
                  _buildHeader(),
                  Divider(height: 1, color: _primary.withOpacity(0.3)),
                  Expanded(
                    child: PageView(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        GestureDetector(
          ...
          child: Text(_model.currentStep > 0 ? 'Voltar' : 'Fechar',
              style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w500, color: _primary)),
        ),
      ...
      child: Form(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('Passageiro', style: FlutterFlowTheme.of(context).titleMedium.override(
                font: GoogleFonts.inter(fontWeight: FontWeight.w700), letterSpacing: 0.0)),
            Text('*Campos Opcionais', style: GoogleFonts.inter(fontSize: 13, color: _secondaryText)),
          ]),
          ...
          Text('Informações Escolares', style: FlutterFlowTheme.of(context).titleMedium.override(
              font: GoogleFonts.inter(fontWeight: FontWeight.w700), letterSpacing: 0.0)),
          const SizedBox(height: 16),
          Row(children: [
            ...
            GestureDetector(onTap: () => _showCadastrarEscola(),
              child: Container(width: 48, height: 48,
                decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: _primary, width: 1.5)),
          ...
          FFButtonWidget(
            ...
            options: FFButtonOptions(width: double.infinity, height: 48,
              ...
              elevation: 0, borderRadius: BorderRadius.circular(_radius)),
          ),
        ]),
      ),
    );
  ...
  Widget _buildStep2Responsavel() {
    return SingleChildScrollView(
      ...
      child: Form(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Text('Responsável', style: FlutterFlowTheme.of(context).titleMedium.override(
                font: GoogleFonts.inter(fontWeight: FontWeight.w700), letterSpacing: 0.0)),
            Text('*Campos Opcionais', style: GoogleFonts.inter(fontSize: 13, color: _secondaryText)),
          ]),
          const SizedBox(height: 16),
          FFButtonWidget(
            ...
            options: FFButtonOptions(width: double.infinity, height: 48,
              ...
              textStyle: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w500, color: _secondaryText),
              elevation: 0, borderRadius: BorderRadius.circular(_radius)),
          ),
          const SizedBox(height: 16),
          Row(children: [
  ...
  Widget _buildStep3Mensalidade() {
    return SingleChildScrollView(
      ...
      child: Form(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Mensalidade', style: FlutterFlowTheme.of(context).titleMedium.override(
              font: GoogleFonts.inter(fontWeight: FontWeight.w700), letterSpacing: 0.0)),
          const SizedBox(height: 8),
          ...
          Text('Obs.: Os dados não são expostos para outros usuários.', style: GoogleFonts.inter(fontSize: 13, color: _secondaryText)),
          const SizedBox(height: 16),
          Row(children: [
          ...
          const SizedBox(height: 24),
          Text('Prazo de Vigência', style: FlutterFlowTheme.of(context).titleMedium.override(
              font: GoogleFonts.inter(fontWeight: FontWeight.w700), letterSpacing: 0.0)),
          const SizedBox(height: 8),
          Text('Informe o prazo de vigência inicial e final:', style: GoogleFonts.inter(fontSize: 14, color: _primaryText)),
          const SizedBox(height: 4),
          Text('Obs.: As mensalidades só serão apresentadas no período selecionado', style: GoogleFonts.inter(fontSize: 13, color: _secondaryText)),
          const SizedBox(height: 16),
          Row(children: [
            Expanded(child: _buildPickerButton(
          ...
          const SizedBox(height: 24),
          FFButtonWidget(
            onPressed: _model.isSaving ? null : () async {
              final success = await _model.saveAll(FFAppState().idUsuario, widget.passageiroId);
              safeSetState(() {});
              if (success && context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(isEditing ? 'Passageiro atualizado com sucesso!' : 'Passageiro cadastrado com sucesso!'),
                      backgroundColor: FlutterFlowTheme.of(context).success));
                context.safePop();
              } else if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(_model.errorMessage ?? 'Erro ao salvar'),
                      backgroundColor: FlutterFlowTheme.of(context).error));
              }
            },
            text: _model.isSaving ? 'Salvando...' : 'Finalizar Cadastro do Passageiro',
            options: FFButtonOptions(width: double.infinity, height: 48,
              color: _model.isSaving ? _primary.withOpacity(0.5) : _primary,
              textStyle: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white),
              elevation: 0, borderRadius: BorderRadius.circular(_radius)),
          ),
        ]),
  ...
  Widget _buildTextField({required TextEditingController controller, required FocusNode focusNode, required String hint,
    ...
    return TextFormField(
      ...
      style: GoogleFonts.inter(fontSize: 15, color: _primaryText),
      decoration: InputDecoration(
        hintText: hint, hintStyle: GoogleFonts.inter(fontSize: 15, color: _secondaryText), counterText: '',
        filled: true, fillColor: _bg, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
        border: OutlineInputBorder(borderRadius: BorderRadius.circular(_radius), borderSide: BorderSide(color: Colors.grey.shade300)),
        enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(_radius), borderSide: BorderSide(color: Colors.grey.shade300)),
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(_radius), borderSide: BorderSide(color: _primary, width: 1.5)),
        errorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(_radius), borderSide: BorderSide(color: FlutterFlowTheme.of(context).error)),
      ),
    );
  }
  ...
  Widget _buildPickerButton({required String label, bool isPlaceholder = false, bool outlined = false, VoidCallback? onTap}) {
    return GestureDetector(onTap: onTap, child: Container(height: 48,
      decoration: BoxDecoration(color: _bg, borderRadius: BorderRadius.circular(_radius),
          border: Border.all(color: outlined ? _primary : Colors.grey.shade300, width: outlined ? 1.5 : 1)),
      alignment: Alignment.center,
      child: Text(label, style: GoogleFonts.inter(fontSize: 15,
          color: isPlaceholder ? _primary : _primaryText, fontWeight: isPlaceholder ? FontWeight.w400 : FontWeight.w500))));
  }

  Widget _buildSexoToggle() {
    return Container(height: 48,
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(_radius), border: Border.all(color: _primary, width: 1.5)),
      child: Row(children: [
        Expanded(child: GestureDetector(onTap: () { _model.sexoMasculino = true; safeSetState(() {}); },
          child: Container(
            decoration: BoxDecoration(color: _model.sexoMasculino == true ? _primary : _bg,
                borderRadius: BorderRadius.horizontal(left: Radius.circular(_radius - 2))),
            alignment: Alignment.center,
            child: Text('Masculino', style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w500,
                color: _model.sexoMasculino == true ? Colors.white : _primary))))),
        Expanded(child: GestureDetector(onTap: () { _model.sexoMasculino = false; safeSetState(() {}); },
          child: Container(
            decoration: BoxDecoration(color: _model.sexoMasculino == false ? _primary : _bg,
                borderRadius: BorderRadius.horizontal(right: Radius.circular(_radius - 2))),
            alignment: Alignment.center,
            child: Text('Feminino', style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w500,
                color: _model.sexoMasculino == false ? Colors.white : _primary))))),
      ]));
  }
  ...
  void _showDatePicker() {
    ...
    showCupertinoModalPopup(context: context, builder: (_) => Container(height: 300, color: _bg,
    ...
    return Container(padding: const EdgeInsets.symmetric(horizontal: 16), height: 44,
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        GestureDetector(onTap: () => Navigator.pop(context),
            child: Text('Cancelar', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w500, color: FlutterFlowTheme.of(context).error))),
  ...
  void _showCadastrarEscola() {
    ...
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: _bg,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(_radius * 2))),
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text('Cadastrar Escola', style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.w600, color: _primary)),
            const SizedBox(width: 60),
          ]),
          Divider(height: 24, color: _primary.withOpacity(0.3)),
          Align(alignment: Alignment.centerLeft,
              child: Text('Escola', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w600, color: _secondaryText))),
          const SizedBox(height: 8),
          TextFormField(controller: controller, autofocus: true, style: GoogleFonts.inter(fontSize: 15, color: _primaryText),
            decoration: InputDecoration(hintText: 'Nome', hintStyle: GoogleFonts.inter(fontSize: 15, color: _secondaryText),
              filled: true, fillColor: _bg, contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(_radius), borderSide: BorderSide(color: Colors.grey.shade300)),
              enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(_radius), borderSide: BorderSide(color: Colors.grey.shade300)),
              focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(_radius), borderSide: BorderSide(color: _primary, width: 1.5)))),
